                ...
                <CardContent>
                    <Tabs defaultValue="new">
                        ...
                        <TabsContent value="new">
                            <ScrollArea className="h-[600px] w-full rounded-md border p-4">
                                {pendingRequests.filter((request) => request.application_state === 'new_application').length === 0 ? (
                                    <p>No pending new applications.</p>
                                ): (
                                pendingRequests.filter((request) => request.application_state === 'new_application').map((request) => (
                        ...
                        <TabsContent value="change">
                            <ScrollArea className="h-[600px] w-full rounded-md border p-4">
                                {pendingRequests.filter((request) => request.application_state === 'change_request').length === 0 ? (
                                    <p>No pending change applications.</p>
                                ): (
                                pendingRequests.filter((request) => request.application_state === 'change_request').map((request) => (
                                    setRequestType(request.application_state),
                                    <Card key={request.application_id} className="mb-4">
                                        <CardHeader>
                                            <CardTitle>Requested By: {request.staff.staff_fname} {request.staff.staff_lname}</CardTitle>
                        ...
                        <TabsContent value="withdraw">
                            <ScrollArea className="h-[600px] w-full rounded-md border p-4">
                                {pendingRequests.filter((request) => request.application_state === 'cancel_request' || request.application_state === 'cancel_one_request').length === 0 ? (
                                    <p>No pending withdraw applications.</p>
                                ): (
                                pendingRequests.filter((request) => request.application_state === 'cancel_request' || request.application_state === 'cancel_one_request').map((request) => (
                                    setRequestType(request.application_state),
                                    <Card key={request.application_id} className="mb-4">
                                        <CardHeader>
                                            <CardTitle>Requested By: {request.staff.staff_fname} {request.staff.staff_lname}</CardTitle>
