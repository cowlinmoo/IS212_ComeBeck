...
class DepartmentRepository:
    ...
    def get_all_department_ids(self) -> List[int]:
        return [department.department_id for department in
                self.db.query(Department).all()]
    ...
    def get_department_by_director_id(self, director_id: int) -> Department:
        department = (self.db.query(Department)
                      .filter(Department.director_id == director_id).first())
        if not department:
            raise HTTPException(status_code=404, detail="Department not found")
        return department

    def get_employees_by_department_id(self,
                                       department_id: int) -> List[Type[Department]]:
