...
class Application(EntityMeta):
    ...
    status = Column(String(20), CheckConstraint("status IN ('approved', 'pending', 'rejected', 'withdrawn', 'superseded')"),
                    nullable=False)
    application_state = Column(Text, CheckConstraint("application_state IN ('new_application', 'cancel_one_request', 'cancel_request','change_request')"),nullable=True)
    ...
    original_application_id = Column(BigInteger, ForeignKey('application.application_id'), nullable=True)
    ...
    original_application = relationship("Application", remote_side="Application.application_id", backref="change_requests")