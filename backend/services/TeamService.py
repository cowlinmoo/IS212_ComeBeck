...
from backend.schemas.BaseSchema import (BaseEmployeeInfo, BaseDepartmentInfo,
                                        BaseEmployeeSchema, BaseTeamInfo)
...
class TeamService:
    ...
    def team_to_schema(self, team: Team) -> TeamSchema:
        ...
        manager = self.employeeRepository.get_employee(team.manager_id) if (
            team.manager_id) else None
        ...
        team_dict = {
            ...
            "department": BaseDepartmentInfo.model_validate(orm_to_dict(department)) if
            department else None,
            "manager": BaseEmployeeInfo.model_validate(orm_to_dict(manager)) if manager
            else None,
            "parent_team": BaseTeamInfo.model_validate(orm_to_dict(team.parent_team))
            if team.parent_team else None,
            "child_teams": [BaseTeamInfo.model_validate(orm_to_dict(child_team)) for
                            child_team in
                            team.child_teams] if team.child_teams else None,
            "members": [BaseEmployeeSchema.model_validate(orm_to_dict(member)) for
                        member in
