...
from backend.schemas.ApplicationSchema import (ApplicationCreateSchema,
                                               ...
                                               ApplicationApproveRejectSchema,
                                               ApprovedApplicationLocationSchema,
                                               ApplicationWithdrawEventSchema)
...
class ApplicationService:
    ...
    def get_application_by_id(self, application_id: int) -> Application:
        ...
        if (self.application_repository
                .get_application_by_application_id(application_id) is None):
            raise HTTPException(
    ...
    def get_employee_approved_application_locations(self, manager_id: int,
                                                    current_user_role:
                                                    EmployeeRole) -> (
            List)[ApprovedApplicationLocationSchema]:
        employee_locations: List[ApprovedApplicationLocationSchema] = []
        approved_applications: List[Application] = (
            self.application_repository.get_applications_by_status(
                status="approved"))
        if current_user_role == EmployeeRole.MANAGER:
            employees = self.employee_repository.get_employees_under_manager(
                manager_id=manager_id)
            approved_applications = [application for application in
                                     approved_applications if application.staff_id in [
        ...
        for approved_application in approved_applications:
            ...
            for event in (self.event_repository
                            .get_event_by_application_id(
    ...
    def get_applications_by_approver_id(self,
                                        approver_id: int) -> List[Type[Application]]:
        # check if employee exists in the database
    ...
    def create_application(self, application: ApplicationCreateSchema,
                           application_state: str) -> Application:
        # check if employee exists in the database
        ...
        if application.recurring:
            if not application.recurrence_type or not application.end_date:
                raise HTTPException(status_code=400,
                                    detail="Recurring applications must have "
                                           "recurrence_type and end_date set")
        ...
        if application.end_date and application.end_date > max_end_date:
            raise HTTPException(status_code=400,
                                detail="End date cannot be more than 3 months away "
        ...
        if application_state == "new_application":
            self.email_service.send_application_creation_emails(application,
                                                                new_application,
                                                                manager,
                                                                employee,
                                                                staff_name)
    ...
    def update_application(self, application_id: int,
                           application: ApplicationCreateSchema) -> Application:
        existing_application = (self.application_repository
        ...
        if existing_application.status == "pending":
            ...
            raise HTTPException(status_code=409,
                                detail="Application has already been withdrawn or "
                                       "rejected")
        else:
            return self.change_request(existing_application, application)

    def withdraw_application_event(self, application_id, event_id,
        ...
        # (either the employee themselves or their manager)
        ...
        if not (is_employee or is_manager):
            raise HTTPException(status_code=403,
                                detail="You are not authorized to withdraw this "
                                       "application")
        ...
        if existing_application.status == "approved" and is_employee:
            return self.cancel_request_one(application_id, event_id,
    ...
    def withdraw_application(self, application_id: int,
                             application: ApplicationWithdrawSchema) -> Application:
        existing_application = (self.application_repository
                                    .get_application_by_application_id(
                                        application_id))
        ...
        withdrawn_application = (self
                                 .application_repository
                                 .withdraw_application(application_id, application))
    ...
    def update_application_status(self, application_id: int,
                                  new_status: str, outcome_reason: str) -> Application:
        return (self.application_repository
                .update_application_status(application_id, new_status, outcome_reason))
    ...
    def reject_old_applications(self):
        ...
        application_ids: List[int] = [int(getattr(app, 'application_id')
                                          if isinstance(app.application_id,
        ...
        for event in events:
            ...
            if event.requested_date < two_months_ago:
                if event.application_id not in event_application_ids:
                    ...
                    application=(self.application_repository
                                 .update_application_status
                                 (event.application_id, 'rejected',
                                  'Application automatically rejected due '
                                  'to old requested date'))
                    rejected_count += 1
                    self.email_service.send_rejection_emails(application, event
                                                             .requested_date)
        print(f"""Rejected {rejected_count} applications with events older than {
              two_months_ago}""")

    def approve_reject_pending_applications(self,
                                            application:
                                            ApplicationApproveRejectSchema
                                            ) -> Application:
        ...
        if existing_application.approver_id != application.approver_id:
            raise HTTPException(status_code=403,
                                detail="You are not authorized to approve this "
                                       "application")
        if existing_application.application_state == "new_application":
            if application.status == "approved":
                ...
                (self.application_repository
                 .update_application_status(application_id, 'rejected',
                                            application.outcome_reason))
            modified_application = (self.application_repository
                                    .get_application_by_application_id(application_id))
            ...
            old_application = (self
                                .application_repository
                                .get_application_by_application_id(
                                    existing_application.original_application_id))
            if application.status == "approved":
                (self.application_repository
                 .update_application_status(existing_application.application_id,
                                            'approved', application.outcome_reason))
                (self.application_repository
                 .update_application_status(old_application.
                                            application_id, 'superseded',
                                            f"Superseded by change"
                                            f" request (Application ID: "
                                            f"{existing_application.application_id})"))
            else:
                (self.application_repository.
                 update_application_status(existing_application.
                                           application_id, 'rejected',
                                           application.outcome_reason))
                (self.application_repository.
                 update_application_status(old_application.
                                           application_id, 'approved',
                                           f"Change request rejected "
                                           f"(Application ID: {existing_application
                                                                   .application_id})"))
            modified_application = (self.application_repository
                                    .get_application_by_application_id(
            ...
            if application.status == "approved":
                event = (self.event_repository.get_first_event_by_application_id
                         (application_id))
                self.event_repository.delete_event(event.event_id)
                self.application_repository.delete_application(application_id)
                (self.email_service
                 .send_cancel_one_request_outcome_emails(event, "approved"))
            elif application.status == "rejected":
                previous_application = (self.application_repository
                                        .get_application_by_application_id(
                                            existing_application
                                            .original_application_id))
                event = (self.event_repository
                         .get_first_event_by_application_id(application_id))
                (self.event_repository
                 .update_application_id(event.event_id,
                                        previous_application.application_id))
                self.application_repository.delete_application(application_id)
                (self.email_service
                 .send_cancel_one_request_outcome_emails(event, "rejected"))
            modified_application = (self.
                                        application_repository.
                                            get_application_by_application_id(
                                                existing_application.
                                                original_application_id))
    ...
    def change_request(self, existing_application: Application, change_request:
        ...
        new_application_dict["original_application_id"] = (existing_application.
                                                           application_id)

        # Create the new application
        new_application = (self.application_repository
                           .create_application(new_application_dict))
        ...
        self.application_repository.update_application_state(
            existing_application.application_id,
            "change_request",
            f"Superseded by change request (Application ID: "
            f"{new_application.application_id})",
            "superseded"
        )
    ...
    def cancel_request_one(self, application_id, event_id, withdraw_reason) -> Event:
        ...
        # create a new application with the event to be removed,
        # and mark the status as pending
        ...
        new_application = (self
                           .create_application(application_to_be_cancelled,
                                               "cancel_one_request"))
        (self.application_repository
         .update_original_application_id(
             new_application.application_id, application_id))
        # remove the event from the application
        new_event = (self.event_repository
                     .get_first_event_by_application_id(new_application.application_id))
